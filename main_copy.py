...
def get_gcode_file(infill_moving_paths, border_width_moving_paths, width, height, file_name='test'):
    layer_height = 0.2
    flow_modifier = 1
    nozzle_diameter = 0.4
    ...
    with open(f'{file_name}_{layers_count}_FW_{flow_modifier}_.gcode', 'w', encoding='utf-8') as f:
        ...
        for j in range(1, (layers_count + 1)):
            ...
            for i in range(len(border_width_moving_paths)):
                path = border_width_moving_paths[i]
                x, y = path[0]
                ...
                if action_type == 'move':
                    ...
                    prev_path = border_width_moving_paths[i - 1]
            ...
            for i in range(len(infill_moving_paths)):
                path = infill_moving_paths[i]
                x, y = path[0]
                x = x_convert_to_cartesian(x, 0, width, width) / 2 + offset
                y = y_convert_to_cartesian(y, 0, height, height) / 2 + offset
                action_type = path[1]
                if action_type == 'move':
                    ...
                    prev_path = infill_moving_paths[i - 1]
                    x_prev, y_prev = prev_path[0]
                    x_prev = x_convert_to_cartesian(x_prev, 0, width, width) / 2 + offset
                    y_prev = y_convert_to_cartesian(y_prev, 0, height, height) / 2 + offset
                    dist = math.dist((x, y), (x_prev, y_prev))
...
def tests():
    ...
    # get_gcode_file(moving_paths, border_moving_paths, width, height, 'test_19')
